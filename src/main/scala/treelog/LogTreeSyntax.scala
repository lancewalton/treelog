...
/** See the [[treelog]] package documentation for a brief introduction to treelog and also,
  ...
  *
  * This trait provides syntax for manipulating `DescribedComputations`. Either: <ul> <li>extend this trait, or</li> <li>define an object
  * with the appropriate Annotation type and import on demand</li> </ul>
  */
trait LogTreeSyntax[Annotation] {
  ...
  /** Create a [[treelog.LogTreeSyntax.DescribedComputation]] representing a failure using the given `description` for both the log tree
    * label and as the content of the `value`, which will be a [[Left]].
    */
  def failure[V](description: String): DescribedComputation[V] =
    failure(description, Leaf(DescribedLogTreeLabel(description, false)))

  /** Create a [[treelog.LogTreeSyntax.DescribedComputation]] representing a success with the given `value` (lifted into a [[Right]]) and
    * the given `description` in the log tree.
    */
  ...
  implicit class LeafSyntax[V](value: V) {
    ...
    /** Create a ''success'' [[treelog.LogTreeSyntax.DescribedComputation]] with `\/-(value)` as the value and a success
      * [[treelog.LogTreeSyntax.DescribedComputation]] using the given `description` function to generate a description for the tree node's
      * label.
      *
      * {{{
      * import treelog.LogTreeSyntaxWithoutAnnotations._
    ...
    /** Create a ''failure'' [[treelog.LogTreeSyntax.DescribedComputation]] with `-\/(description)` as the value and a failure
      * [[treelog.LogTreeSyntax.DescribedComputation]] with the given `description`.
      *
      * {{{
      * import treelog.LogTreeSyntaxWithoutAnnotations._
      * import scalaz.syntax.show._
    ...
    /** Create a ''failure'' [[treelog.LogTreeSyntax.DescribedComputation]] using the given `description` function to generate a description
      * for the tree node's label and for the `DescribedComputations` value (i.e. the value will be `\/-(description(value))`.
      *
      * {{{
      * import treelog.LogTreeSyntaxWithoutAnnotations._
      * import scalaz.syntax.show._
  ...
  /** Syntax for treating booleans as indicators of success or failure in a computation.
    *
    * The simplest usage is something like: `myBoolean ~>? "Is my boolean true?"`. The 'value' and log tree of the returned
    * [[treelog.LogTreeSyntax.DescribedComputation]] will indicate success or failure depending on the value of `myBoolean`.
    */
  implicit class BooleanSyntax(b: Boolean) {
    ...
    /** Use different descriptions for the `true` and `false` cases. Note that unlike `'if'` the `false` / failure description is the first
      * parameter and the `true` / success description is the second parameter. This is to maintain consistency with
      * [[treelog.LogTreeSyntax.OptionSyntax OptionSyntax]] and [[treelog.LogTreeSyntax.EitherSyntax EitherSyntax]].
      *
      * If the boolean is `true` the 'value' of the returned DescribedComputation will be `\/-(true)`, otherwise, the 'value' will be
      * `-\/(description)`.
      */
  ...
  implicit class OptionSyntax[V](option: Option[V]) {
    ...
    /** Use different descriptions for the `Some` and `None` cases.
      *
      * If the option is `Some(x)` the 'value' of the returned DescribedComputation will be `\/-(x)`, otherwise, the 'value' will be
      * `-\/(noneDescription)`.
      */
    ...
    /** Use different descriptions for the `Some` and `None` cases, providing the boxed `Some` value to the function used to produce the
      * description for the `Some` case, so that it can be included in the description if you wish.
      *
      * If the option is `Some(x)` the 'value' of the returned DescribedComputation will be `\/-(x)`, otherwise, the 'value' will be
      * `-\/(noneDescription)`.
      */
    ...
    /** Return a default [[treelog.LogTreeSyntax.DescribedComputation]] if `option` is a `None`.
      *
      * If the option is `Some(x)` the 'value' of the returned DescribedComputation will be `\/-(Some(x))`, otherwise, the returned
      * [[treelog.LogTreeSyntax.DescribedComputation]] will be `dflt`.
      */
    def ~>|[B](
  ...
  /** Syntax for treating `scalaz.\/` as signifiers of success or failure in a computation.
    *
    * The simplest usage is something like: `myEither ~>? "Do I have the right?"`. The 'value' and log tree of the returned
    * [[treelog.LogTreeSyntax.DescribedComputation]] will indicate success or failure depending on the value of `myEither`.
    */
  implicit class EitherSyntax[V](either: Either[String, V]) {
    ...
    /** Use the same description regardless of whether `either` is a `\/-` or a `-\/`. Equivalent to: `~>?((error: String) => s"$description
      * \- $error", description)`
      */
  ...
  implicit class BranchLabelingSyntax(description: String) {
    ...
    /** Create a new branch given a monadic, traversable 'container' `F[DescribedComputation[Value]]`, 'sequence' it to create a
      * `DescribedComputation[F[Value]]`, and give the new `DescribedComputation's` log tree a new root node, with the given `description`
      * and whose children are the trees in the `F[DescribedComputation[Value]]`.
      *
      * For example, if we evaluate this method with `F` instantiated as `List`, we would turn a `List[DescribedComputation[Value]]` into a
      * `DescribedComputation[List[Value]]`, such that the `List[Value]` which is the result's 'value' is obtained by extracting the 'value'
      * from each [[treelog.LogTreeSyntax.DescribedComputation]] in the `describedComputations` parameter. Likewise, the child nodes of the
      * returned log tree root node are obtained by extracting the log tree from each of the `describedComputations`.
      *
      * The ''success'' status of the returned `DescribedComputations` log tree is `true` if all of the children are successful. It is
      * `false` otherwise.
      */
    ...
    /** As ~< but folding over the resulting F[Value] to yield R and return a DescribedComputation[R] with all the logs.
      *
      * For example, given l = List[DescribedComputation[Int]], and f = List[Int] => Int (say summing the list), then `"Sum" ~&lt;+(l, f)`
    ...
    /** If `dc` has a log tree with an undescribed root node, give the root node the `description` but otherwise leave it unchanged. If the
      * log tree has a described root node, create a new root node above the existing one and give the new root node the `description`. In
      * both cases preserve the `value` and success/failure status.
      */
