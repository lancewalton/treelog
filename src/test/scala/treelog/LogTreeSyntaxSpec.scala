...
class LogTreeSyntaxSpec extends RefSpec with Matchers {
  ...
  object `~>/ must` {
    ...
    def `return failure and a log tree describing the fold as far as it got`() = {
      ...
      assert(
        result.value.written ===
          node("Bar", false, node("Initial Value", true), node("x=1, result=1", true), node("x=2, result=3", true), node("No", false))
      )
  ...
  private def node(success: Boolean, children: Tree[LogTreeLabel[Nothing]]*) = Node(UndescribedLogTreeLabel(success), children.toLazyList)
}
