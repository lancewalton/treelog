...
class LogTreeSyntaxSpec extends RefSpec with Matchers {
  ...
  object `leaf creation with ~> and a function must` {
    def `do the same as 'success' after applying the function to the value`() =
      assert((1 ~> (x => s"Foo: $x")).value.run === success(1, "Foo: 1").value.run)
  }
  ...
  object `leaf creation with ~>! and a function must` {
    def `do the same as 'failure' after applying the function to the value`() =
      assert((1 ~>! (x => s"Foo: $x")).value.run === failure("Foo: 1").value.run)
  }
  ...
  object `boolean ~>? with a description must` {
    ...
    def `do the same as 'failure' when the boolean is false`() = assert(
      (false ~>? "Foo").value.run === failure("Foo").value.run
    )
  }
  ...
  object `Option ~>? with a description must` {
    def `do the same as 'success' when the Option is Some`() = assert(
      (Some(2) ~>? "Foo").value.run === success(2, "Foo").value.run
    )
    def `do the same as 'failure' when the Option is None`() = assert(
      (none[Int] ~>? "Foo").value.run === failure("Foo").value.run
    )
  }

  object `Option ~>? with a noneDescription and a someDescription must` {
    def `do the same as 'success' when the Option is Some`() =
      assert((Some(2) ~>? ("Foo", "Bar")).value.run === success(2, "Bar").value.run)
    def `do the same as 'failure' when the Option is None`() = assert(
      (none[Int] ~>? ("Foo", "Bar")).value.run === failure("Foo").value.run
    )
  }

  object `Option ~>? with a noneDescription and a function for someDescription must` {
    def `do the same as 'success' when the Option is Some`() =
      assert((Some(2) ~>? ("Foo", x => "Bar: " + x)).value.run === success(2, "Bar: 2").value.run)
    def `do the same as 'failure' when the Option is None`() =
      assert((none[Int] ~>? ("Foo", x => "Bar: " + x)).value.run === failure("Foo").value.run)
  }
  ...
  object `~>/ must` {
    ...
    def `return success with the folded value  and a log tree describing the fold when all parts are successes`() = {
      val result =
        List(1, 2, 3) ~>/ ("Foo", 0 ~> "Initial Value", (acc: Int, x: Int) => (acc + x) ~> (t => s"x=$x, result=$t"))
      assert(
  ...
  object `Hoisting a branch must` {
    ...
    def `create a new parent above the existing branch and give it the description when the hoisted branch has a description`() = {
      ...
      assert(result.value.value === Right(3))
      assert(
        result.value.written === node("Grandparent", true, node("Parent", true, node("One", true), node("Two", true)))
      )
    }
